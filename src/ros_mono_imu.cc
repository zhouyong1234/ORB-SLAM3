...
//  ros::Subscriber sub_img0 = n.subscribe("/camera/image_raw", 100, &ImageGrabber::GrabImage,&igb);
  ros::Subscriber sub_img0 = n.subscribe("/camera/image/image_raw", 100, &ImageGrabber::GrabImage,&igb);
  ...
  SLAM.Shutdown();
  SLAM.SaveKeyFrameTrajectoryTUM("/home/lin/output/orb3_imu.txt");
...
// cv::Mat ImageGrabber::GetImage(sensor_msgs::Image &img_msg)
// {
...
//     cv::Mat cvCompImg;
//     cv::resize(cvImg,cvCompImg,cv::Size(640,480));
//     img_msg = cv_bridge::CvImage(std_msgs::Header(),"mono8",cvCompImg).toImageMsg();
//   }
	...
	if(!mpImuGb->imuBuf.empty())
            {
                ...
                while(!mpImuGb->imuBuf.empty() && mpImuGb->imuBuf.front()->header.stamp.toSec()-TIME_OFFSET<=tIm)
                {
                    ...
                    cv::Point3f gyr(mpImuGb->imuBuf.front()->angular_velocity.x, mpImuGb->imuBuf.front()->angular_velocity.y, mpImuGb->imuBuf.front()->angular_velocity.z);
                 //"IMU数据： "
                 //   cout<<"IMU数据： "<< t<<" "<<acc.x<<" "<<acc.y<<" "<<acc.z<<" "<<gyr.x<<" "<<gyr.y<<" "<<gyr.z<<endl;
